...
const TARGET_TEAM = process.env.TARGET_TEAM; // 'A' ou 'B'

// Validação de segurança
if (!TARGET_TEAM || (TARGET_TEAM !== 'A' && TARGET_TEAM !== 'B')) {
  console.error("ERRO: TARGET_TEAM não definido corretamente (A ou B).");
  process.exit(1);
}

// Definição das coleções baseado na Turma
const colEmployeesName = TARGET_TEAM === 'A' ? 'turma a' : 'turma b';
const colRegistrosName = TARGET_TEAM === 'A' ? 'registrosDSS A' : 'registrosDSS B';
...
async function gerarRelatorio() {
  ...
  // A) Ler dados dos 'employees' (agora turma a ou b)
  try {
  ...
  // B) Ler dados dos 'registrosDSS' (A ou B)
  try {
...
async function enviarEmail(htmlRelatorio) {
  ...
  // Assunto dinâmico (TURMA A ou TURMA B)
