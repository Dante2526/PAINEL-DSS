...
const LayoutSelectionScreen: React.FC<LayoutSelectionScreenProps> = ({ onSelect, isDarkMode, onToggleDarkMode, onBack, selecionadaTurma }) => {
    return (
        <div className="bg-light-bg-secondary dark:bg-dark-bg h-[100dvh] w-full text-light-text dark:text-dark-text transition-colors flex flex-col items-center p-4 pt-[env(safe-area-inset-top)] pb-[calc(env(safe-area-inset-bottom)+1.5rem)] md:pb-[max(env(safe-area-inset-bottom),1.5rem)] overflow-hidden">
            <button 
                onClick={onBack}
            ...
            <div className="flex-grow flex flex-col w-full mt-10 md:mt-8 pb-2">
                <main className="flex-grow flex flex-col items-center justify-center text-center m-auto w-full max-w-2xl px-2">
                    ...
                    <div className="flex flex-col md:grid md:grid-cols-2 gap-4 w-[300px] md:w-full md:max-w-xl mx-auto mb-3 md:mb-6">
                        {/* Padrão */}
                        <button
                            onClick={() => onSelect('standard')}
                            className="flex flex-col items-center justify-center p-5 md:p-8 bg-light-card dark:bg-dark-card rounded-3xl shadow-lg border border-transparent hover:border-blue-500 transform hover:-translate-y-1 transition-all duration-300 group focus:outline-none focus:ring-4 focus:ring-blue-500 w-full"
                        >
                        ...
                        <button
                            ...
                            className="flex flex-col items-center justify-center p-5 md:p-8 bg-light-card dark:bg-dark-card rounded-3xl shadow-lg border border-transparent hover:border-cyan-500 transform hover:-translate-y-1 transition-all duration-300 group focus:outline-none focus:ring-4 focus:ring-cyan-500 w-full relative overflow-hidden"
                        >
